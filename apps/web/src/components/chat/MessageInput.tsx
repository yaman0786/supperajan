...
export function MessageInput() {
  ...
  const { isThinking, isStreaming, addMessage } = useAssistantStore();
  ...
  const handleSend = useCallback(() => {
    ...
    // TODO (Phase 5): emit user.message event over WebSocket
  }, [canSend, value, addMessage]);
  ...
  return (
    <div className="flex items-end gap-2 p-3">
      <textarea
        ...
        onInput={handleInput}
        placeholder={isDisabled ? 'Assistant is responding...' : 'Message Süpperajan...'}
        disabled={isDisabled}
        rows={1}
